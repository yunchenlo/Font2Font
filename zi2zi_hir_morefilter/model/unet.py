...
class UNet(object):
    ...
    def discriminator(self, image, is_training, reuse=False):
        with tf.variable_scope("discriminator"):
            ...
            h2 = lrelu(batch_norm(conv2d(h1, self.discriminator_dim * 8, scope="d_h2_conv"),
                                  is_training, scope="d_bn_2"))
            h3 = lrelu(batch_norm(conv2d(h2, self.discriminator_dim * 16, sh=1, sw=1, scope="d_h3_conv"),
                                  is_training, scope="d_bn_3"))

            # Hierarchical Adversarial Discriminator
            d_fc = fc(tf.reshape(h3, [self.batch_size, -1]), 1, scope="d_fc")
            cat_fc0 = fc(tf.reshape(h0, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc0")
            cat_fc1 = fc(tf.reshape(h1, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc1")
            cat_fc2 = fc(tf.reshape(h2, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc2")
            cat_fc3 = fc(tf.reshape(h3, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc3")
    ...
    def get_model_id_and_dir(self):
        model_id = "experiment_%d_batch_%d_hier_morefilter" % (self.experiment_id, self.batch_size)

...
class UNet(object):
    ...
    def decoder(self, encoded, encoding_layers, ids, inst_norm, is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            def decode_layer(x, output_width, output_filters, layer, enc_layer, dropout=False, do_concat=True, norm=True, kh=5, kw=5, sh=2, sw=2):
            ...
            d8 = decode_layer(d7, s, self.output_filters, layer=8, enc_layer=None, do_concat=False, norm=False)

            output = tf.nn.tanh(d8)  # scale to (-1, 1)

            # Hierachical term 
            T1_pre = decode_layer(d5, s, self.output_filters, layer=9, enc_layer=None, do_concat=False, norm=False, kh=5, kw=5, sh=8, sw=8)
            T1 = tf.nn.tanh(T1_pre)
            T2_pre = decode_layer(d6, s, self.output_filters, layer=10, enc_layer=None, do_concat=False, norm=False, kh=5, kw=5, sh=4, sw=4)
            T2 = tf.nn.tanh(T2_pre)
    ...
    def generator(self, images, embeddings, embedding_ids, inst_norm, is_training, reuse=False):
        ...
        output, T1, T2 = self.decoder(embedded, enc_layers, embedding_ids, inst_norm, is_training=is_training, reuse=reuse)
        return output, e8, T1, T2
    ...
    def discriminator(self, image, is_training, reuse=False):
        with tf.variable_scope("discriminator"):
            ...
            h2 = lrelu(batch_norm(conv2d(h1, self.discriminator_dim * 2, scope="d_h2_conv"),
                                  is_training, scope="d_bn_2"))
            h3 = lrelu(batch_norm(conv2d(h2, self.discriminator_dim * 4, scope="d_h3_conv"),
                                  is_training, scope="d_bn_3"))
            h4 = lrelu(batch_norm(conv2d(h3, self.discriminator_dim * 4, scope="d_h4_conv"),
            ...
            d_fc = fc(tf.reshape(h5, [self.batch_size, -1]), 1, scope="d_fc")
            cat_fc0 = fc(tf.reshape(h0, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc0")
            cat_fc1 = fc(tf.reshape(h2, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc1")
            cat_fc2 = fc(tf.reshape(h4, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc2")
            cat_fc3 = fc(tf.reshape(h5, [self.batch_size, -1]), self.embedding_num, scope="d_cat_fc3")
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        fake_B, encoded_real_A, fake_B_T1, fake_B_T2 = self.generator(real_A, embedding, embedding_ids, is_training=is_training,
                                                inst_norm=inst_norm)
        
        #real_AB = tf.concat([real_A, real_B, real_B, real_B], 3)
        #fake_AB = tf.concat([real_A, fake_B, fake_B_T1, fake_B_T2], 3)
        real_AB = tf.concat([real_A, real_B], 3)
        fake_AB = tf.concat([real_A, fake_B], 3)
        fake_AB_T1 = tf.concat([real_A, fake_B_T1], 3)
        fake_AB_T2 = tf.concat([real_A, fake_B_T2], 3)
        ...
        fake_D0_T1, fake_cat_D0_T1, fake_cat_D1_T1, fake_cat_D2_T1, fake_cat_D3_T1 = self.discriminator(fake_AB_T1, is_training=is_training, reuse=True)
        fake_D0_T2, fake_cat_D0_T2, fake_cat_D1_T2, fake_cat_D2_T2, fake_cat_D3_T2 = self.discriminator(fake_AB_T2, is_training=is_training, reuse=True)
        ...
        real_category_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=real_cat_D0,
                                                                                    labels=true_labels))
        real_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=real_cat_D1,
                                                                                    labels=true_labels))
        real_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=real_cat_D2,
                                                                                    labels=true_labels))
        real_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=real_cat_D3,
                                                                                    labels=true_labels))                                                                           

        fake_category_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D0,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D1,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D2,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D3,
                                                                                    labels=true_labels))

        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D0_T1,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D1_T1,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D2_T1,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D3_T1,
                                                                                    labels=true_labels))
        
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D0_T2,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D1_T2,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D2_T2,
                                                                                    labels=true_labels))
        fake_category_loss += tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fake_cat_D3_T2,
                                                                                    labels=true_labels))
        ...
        if no_target_source:
            ...
            no_target_B, encoded_no_target_A, no_target_B_T1, no_target_B_T2 = self.generator(no_target_A, embedding, no_target_ids,
                                                                                                is_training=is_training,
                                                                                                inst_norm=inst_norm, reuse=True)
            no_target_labels = tf.reshape(tf.one_hot(indices=no_target_ids, depth=self.embedding_num),
                                          shape=[self.batch_size, self.embedding_num])
            no_target_AB = tf.concat([no_target_A, no_target_B, no_target_B_T1, no_target_B_T2], 3)
            ...
            no_target_D, no_target_D_logits, no_target_category_logits = self.discriminator(no_target_AB,
                                                                                            ...
                                                                                            reuse=True)
            '''
            no_target_D0, no_target_cat_D0, no_target_cat_D1, no_target_cat_D2, no_target_cat_D3 = self.discriminator(no_target_AB,
                                                                                                                        is_training=is_training,
                                                                                                                        reuse=True)
            ...
            no_target_category_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=no_target_cat,
                                                        labels=no_target_labels)) * self.Lcategory_penalty
    ...
    def get_model_id_and_dir(self):
        model_id = "experiment_%d_batch_%d_hier_dis" % (self.experiment_id, self.batch_size)
